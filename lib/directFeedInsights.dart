...
class DirectFeedingInsightsPage extends StatefulWidget {
  ...
  @override
  _DirectFeedingInsightsPageState createState() =>
      _DirectFeedingInsightsPageState();
}
...
class _DirectFeedingInsightsPageState extends State<DirectFeedingInsightsPage> {
  ...
  Future<Map<String, dynamic>> _fetchInsights() async {
    try {
      ...
      final List<Map<String, dynamic>> feedingData =
  ...
  Map<String, dynamic> _calculateInsights(
    ...
    feedingData.sort((a, b) => DateTime.parse(b['startTime'])
    ...
    double averageQuantityOz =
        bottleFeedings > 0 ? totalQuantityOz / bottleFeedings : 0;
    Duration averageDuration = totalFeedings > 0
        ? Duration(seconds: totalDurationSeconds ~/ totalFeedings)
        : Duration.zero;
  ...
  Map<String, dynamic> _getFeedingRecommendation(int ageInMonths) {
    if (ageInMonths < 1) {
      return {
        ...
        'recommendation':
            'Feed your baby 8-12 times per day, about 2-3 ounces (60-90 ml) per feeding.',
      };
    } else if (ageInMonths < 3) {
      return {
        'recommendedFrequency': 7.0,
        'recommendedVolumeOz':
            3.5, // 105 ml (average of 90-120 ml) converted to oz
        'recommendedVolumeRange': '3-4 oz',
        'recommendation':
            'Feed your baby 6-8 times per day, about 3-4 ounces (90-120 ml) per feeding.',
      };
    } else if (ageInMonths <= 6) {
      return {
        'recommendedFrequency': 6.0,
        'recommendedVolumeOz':
            5.0, // 150 ml (average of 120-180 ml) converted to oz
        'recommendedVolumeRange': '4-6 oz',
        'recommendation':
            'Feed your baby 5-6 times per day, about 4-6 ounces (120-180 ml) per feeding.',
      };
    } else {
      return {
        ...
        'recommendation':
            'Feed your baby 4-5 times per day, about 6-8 ounces (180-240 ml) per feeding.',
      };
    }
  }
  ...
  Map<String, dynamic> _getBreastfeedingRecommendation(int ageInMonths) {
    if (ageInMonths < 1) {
      return {
        ...
        'note':
            'Newborns may take longer to feed as they are still learning how to latch and suck effectively. They may also fall asleep during feedings and need to be gently awakened to finish.',
      };
    } else if (ageInMonths < 3) {
      return {
        'ageGroup': '1-3 Months',
        ...
        'note':
            'As babies grow, they become more efficient at sucking, reducing the time needed per session.',
      };
    } else if (ageInMonths < 6) {
      return {
        'ageGroup': '3-6 Months',
        ...
        'note':
            'At this stage, babies can usually get enough milk within shorter periods due to stronger suction.',
      };
      ...
      return {
        ...
        'note':
            'Babies are starting to eat solid foods around this time, which can decrease the need for frequent and long breastfeeding sessions.',
      };
    } else {
      return {
        ...
        'note':
            'Breastfeeding can become more of a comfort or bonding activity rather than a primary source of nutrition.',
      };
    }
  }
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            const SizedBox(height: 16),
            _buildOverviewItem('Total Feedings',
                insights['totalFeedings']?.toString() ?? 'N/A'),
            _buildOverviewItem('Breast Feedings',
                insights['breastFeedings']?.toString() ?? 'N/A'),
            _buildOverviewItem('Bottle Feedings',
                insights['bottleFeedings']?.toString() ?? 'N/A'),
            _buildOverviewItem(
              'Total Feeding Time',
                      ...
                      PieChartData(
                        sections: [
                          PieChartSectionData(
                            ...
                            title:
                                '${(breastFeedings / total * 100).toStringAsFixed(1)}%',
                            radius: 70,
                          ...
                          PieChartSectionData(
                            ...
                            title:
                                '${(bottleFeedings / total * 100).toStringAsFixed(1)}%',
                            radius: 70,
                            titleStyle: const TextStyle(
                                color: Colors.white,
                                fontWeight: FontWeight.bold),
  ...
  Widget _buildFeedingProgressBar(Map<String, dynamic> insights) {
    Map<String, dynamic> recommendation =
        _getFeedingRecommendation(widget.babyAgeInMonths);
    ...
    Map<String, dynamic> breastRecommendation =
        _getBreastfeedingRecommendation(widget.babyAgeInMonths);
    List<String> recommendedDuration =
        breastRecommendation['sessionDuration'].split('-');
    int minRecommendedDuration =
        int.parse(recommendedDuration[0].replaceAll(RegExp(r'[^0-9]'), ''));
    int maxRecommendedDuration =
        int.parse(recommendedDuration[1].replaceAll(RegExp(r'[^0-9]'), ''));
    int recommendedDurationSeconds =
        ((minRecommendedDuration + maxRecommendedDuration) / 2 * 60).round();

    double frequencyProgress =
        (insights['totalFeedings'] as int) / recommendedFrequency;

    double? lastBottleFeedingQuantityOz =
        insights['lastBottleFeedingQuantityOz'] as double?;
    int? lastBreastFeedingDurationSeconds =
        insights['lastBreastFeedingDurationSeconds'] as int?;
    ...
    if (lastBreastFeedingDurationSeconds != null) {
      breastDurationProgress =
          lastBreastFeedingDurationSeconds / recommendedDurationSeconds;
      breastDurationText =
          _formatDuration(Duration(seconds: lastBreastFeedingDurationSeconds));
    }
  ...
  Widget _buildFeedingDurationChart(
      Map<String, dynamic> insights, TimePeriod selectedTimePeriod) {
    final Map<String, List<Map<String, dynamic>>> feedingsByDay =
        Map<String, List<Map<String, dynamic>>>.from(insights['feedingsByDay']);
    ...
    if (selectedTimePeriod == TimePeriod.today) {
      ...
      for (DateTime date = startDate;
        ...
        if (breastCount > 0) {
          ...
          breastFeedingSpots
              .add(FlSpot(dayIndex.toDouble(), avgBreastDuration));
        ...
        if (bottleCount > 0) {
          ...
          bottleFeedingSpots
              .add(FlSpot(dayIndex.toDouble(), avgBottleDuration));
    ...
    Map<String, dynamic> breastfeedingRecommendation =
        _getBreastfeedingRecommendation(widget.babyAgeInMonths);
    List<String> recommendedDuration =
        breastfeedingRecommendation['sessionDuration'].split('-');

    double minRecommendedDuration =
        double.parse(recommendedDuration[0].replaceAll(RegExp(r'[^0-9]'), ''));
    double maxRecommendedDuration =
        double.parse(recommendedDuration[1].replaceAll(RegExp(r'[^0-9]'), ''));
            ...
            Row(
              ...
              children: [
                _buildFilterChip(
                    ...
                    _showBreastFeeding,
                    () => setState(
                        () => _showBreastFeeding = !_showBreastFeeding)),
                _buildFilterChip(
                    'Bottle',
                    Colors.blue,
                    _showBottleFeeding,
                    () => setState(
                        () => _showBottleFeeding = !_showBottleFeeding)),
              ],
            ),
            const SizedBox(height: 16),
            SizedBox(
  ...
  Widget _buildFilterChip(
      String label, Color color, bool isSelected, VoidCallback onSelected) {
    return FilterChip(
      label: Text(label),
      selected: isSelected,
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            const SizedBox(height: 8),
            Text(
                'Recommended Session Duration: ${recommendation['sessionDuration']}'),
            const SizedBox(height: 8),
            ...
            if (insights['averageDuration'] != null)
              Text(
                  '- Your average feeding duration is ${(insights['averageDuration'] as Duration).inMinutes} minutes. ${insights['averageDuration'].inMinutes < extractFirstNumber(recommendation['sessionDuration'])! ? 'Try to extend feeding sessions if possible.' : 'This is within the recommended range.'}'),
            if (insights['totalFeedings'] != null &&
                minRecommendedFrequency != null)
              Text(
                  '- You\'re currently feeding ${insights['totalFeedings']} times per day. ${insights['totalFeedings'] < minRecommendedFrequency ? 'Consider increasing feeding frequency if your baby seems hungry.' : 'This is within the recommended range.'}'),
          ],
        ),
      ),
    );
